...
def generate_social_ad_image():
    ...
    if template == 'warranty':
        wt_img = Image.new('RGB', (W, H), NAVY)
        wt_draw = ImageDraw.Draw(wt_img)
        wt_draw.rectangle([0,0,W,8],fill=GREEN)
        if car_img:
            car_copy = car_img.convert('RGB')
            scale = max(W/car_copy.width, 420/car_copy.height)
            nw,nh = int(car_copy.width*scale),int(car_copy.height*scale)
            car_copy = car_copy.resize((nw,nh))
        region = Image.new('RGB',(W,380),NAVY)
        region.paste(car_copy,((W-nw)//2,(380-nh)//2))
        wt_img.paste(region,(0,100))
        wt_draw.rounded_rectangle([W//2-260,130,W//2+260,200],radius=35,fill=(234,179,8))
        wt_draw.text((W//2,55),'FACTORY WARRANTY',font=font_bold_md,fill=NAVY,anchor='mm')
        if profile_img:
            ...
            wt_img.paste(pr,(30,25),mask)
            wt_draw.ellipse([24,19,116,111],outline=GREEN,width=3)
        wt_draw.text((40,580),vehicle_name,font=font_bold_lg,fill=WHITE,anchor='lm')
        wt_draw.text((40,650),price,font=font_price,fill=GREEN,anchor='lm')
        wt_draw.text((40,730),'Still Under Factory Warranty',font=font_bold_sm,fill=(234,179,8),anchor='lm')
        if days_left>0:
            bc=(220,38,38) if days_left<=2 else (249,115,22) if days_left<=4 else GREEN
            wt_draw.rounded_rectangle([W-240,620,W-20,666],radius=23,fill=bc)
            wt_draw.text((W-130,643),f'{days_left} Days Left',font=font_bold_sm,fill=WHITE,anchor='mm')
        wt_draw.rectangle([0,800,W,920],fill=(20,30,48))
        ...
        wt_draw.rectangle([0,920,W,1010],fill=(15,23,42))
